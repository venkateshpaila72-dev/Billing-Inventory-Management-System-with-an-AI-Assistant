...
const MySales = () => {
  ...
  useEffect(() => {
    ...
    setFiltered(sales.filter(s => s.bill_number.toLowerCase().includes(q)));
  }, [search, sales]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-gray-800 font-bold text-2xl">My Sales</h1>
        <p className="text-gray-400 text-sm mt-0.5">{sales.length} bills generated by you</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-gray-400">
            ...
            <p className="text-sm">{search ? "No bills match your search" : "You haven't made any sales yet"}</p>
          </div>
        ) : (
              ...
              <tbody>
                {filtered.map((s, i) => (
                  <tr key={s.id} className="hover:bg-gray-50 transition-colors cursor-pointer"
                    onClick={() => navigate(`/staff/sales/${s.id}`)}>
                    ...
                    <td className="table-cell">
                      <button onClick={e => { e.stopPropagation(); navigate(`/staff/sales/${s.id}`); }}
